...
/**
 * Controlador para la gestión de cursos plantilla.
 * Responsabilidades:
 * - Coordinar las operaciones entre la vista y el servicio
 * - Gestionar servicios de UI (imágenes, audio)
 * - Proporcionar una interfaz simplificada para la capa de presentación
 * - Manejar la inicialización y configuración del sistema
 */
public class ControladorCursoPlantilla {
    ...
    public ControladorCursoPlantilla(ServicioCursoPlantilla servicioCursoPlantilla,
                                   ImageService servicioImagenes,
                                   AudioService reproductor,
    ...
    public CursoEnProgreso getCursoEnProgreso(CursoPlantilla cursoPlantilla, 
                                            ...
                                            Usuario usuario) {
        try {
            return servicioCursoPlantilla.crearCursoEnProgreso(cursoPlantilla, aprendizajeSeleccionado, usuario);
        } catch (Exception e) {
            System.err.println("Error al crear curso en progreso: " + e.getMessage());
            return null;
        }
    ...
    public CursoPlantilla crearCurso(String nombre, String descripcion, String objetivos) {
        try {
            String propietario = controladorUsuario.getUsuarioActual().getNickname();
    ...
    /**
     * Obtiene el servicio de cursos para operaciones avanzadas
     * (Solo para casos especiales, generalmente no debería usarse desde la UI)
     */
